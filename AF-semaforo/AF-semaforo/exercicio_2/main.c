...
int total_veiculos;
int veiculos_turno;
sem_t cont_ilha, ilha_cont, saida_ponte;
int conta_carros = 0;
...
void ponte_inicializar(int total_veiculos, int veiculos_turno) {	
	...
	// semáforos do lado do continente
	sem_init(&cont_ilha, 0, veiculos_turno);
	// semáforos do lado da ilha
	sem_init(&ilha_cont, 0, 0);
	sem_init(&saida_ponte, 0, 1); // binário
...
void ponte_entrar(veiculo_t *v) {
	// ToDo: IMPLEMENTAR!
	if (v -> cabeceira) // se carro estiver indo entrar pela ilha
		sem_wait(&ilha_cont);
	else
		sem_wait(&cont_ilha);
}
...
void ponte_sair(veiculo_t *v) {
	// ToDo: IMPLEMENTAR!
	sem_wait(&saida_ponte);
	conta_carros++;
	if (conta_carros == veiculos_turno) {
		conta_carros = 0;
