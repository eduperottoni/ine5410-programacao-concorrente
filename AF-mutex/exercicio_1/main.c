...
pthread_mutex_t incrementor_mutex;
...
void *incrementor(void *arg) {
    ...
    for (int i = 0; i < n_loops; i++) {
        // ocupa o mutex
        pthread_mutex_lock(&incrementor_mutex);
        contador_global += 1;
        // desocupa o mutex
        pthread_mutex_unlock(&incrementor_mutex);
...
int main(int argc, char* argv[]) {
    ...
    // Inicializa o mutex
    pthread_mutex_init(&incrementor_mutex, NULL);
    ...
    // Destrói o mutex
    pthread_mutex_destroy(&incrementor_mutex);
